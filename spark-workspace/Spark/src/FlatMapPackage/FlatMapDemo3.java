...
public class FlatMapDemo3
{
	...
	public static void main(String[] args) 
	{
		...
		JavaRDD<String> logRecord = sc.parallelize(inputData);

		JavaRDD<String> logFields = logRecord.flatMap(value -> Arrays.asList(value.split(" ")).iterator());

        JavaRDD<String> filteredFields = logFields.filter(value -> value.length() > 3);
        //JavaRDD<String> filteredFields = logFields.filter(value -> false);
        
		filteredFields.collect().forEach(System.out::println);
